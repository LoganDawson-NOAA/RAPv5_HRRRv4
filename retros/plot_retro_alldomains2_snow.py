...
# Get and shift NOHRSC lat/lons
get_nohrsc_latlon = True
...
#for j in range(len(vtime_list[0:7])):
    ...
    # Snow depth
    snod_1 = data1.select(name='Snow depth')[0].values * 39.3701
    snod_2 = data2.select(name='Snow depth')[0].values * 39.3701
    snod_dif = snod_2 - snod_1

    snod_list_1.append(snod_1)
    snod_list_2.append(snod_2)
    snod_dif_list.append(snod_dif)

    # Variable SLR
    snow_1 = data1.select(name='Total snowfall')[0].values * 39.3701
    snow_2 = data2.select(name='Total snowfall')[0].values * 39.3701
    ...
    if int(vtime[8:10])%6 == 0:
        ...
        if get_nohrsc_latlon:
            lat1 = data4[1]['latitudeOfFirstGridPointInDegrees']
            lon1 = data4[1]['longitudeOfFirstGridPointInDegrees']
            try:
                nx = data4[1]['Nx']
                ny = data4[1]['Ny']
            except:
                nx = data4[1]['Ni']
                ny = data4[1]['Nj']
            dx = data4[1]['DxInMetres']
            dy = data4[1]['DyInMetres']
            pj = pyproj.Proj(data4[1].projparams)
            ...
            x,y = np.meshgrid(x,y)
            lon4_shift, lat4_shift = pj(x, y, inverse=True)

            # Skip this step now that we have the shifted grid
            get_nohrsc_latlon = False
...
########################################
  # Plot total accumulated snowfall
########################################
  if (fhr%6 == 0):
    t1 = time.clock()
    print('Working on total accumulated snowfall for '+dom)

    snow_1 = snow_list_1[fhr]
    snow_2 = snow_list_2[fhr]
    snow_dif = snow_dif_list[fhr]
    ...
    for ax in axes:
      ...
      if par == 1:
        cs_1 = m.pcolormesh(x,y,snow_1,cmap=cm,vmin=0.1,norm=norm,ax=ax)
        ...
        ax.text(.5,1.03,para_str+' Variable SLR Accumulated Snowfall ('+units+') \n initialized: '+itime+' valid: '+vtime + ' (f'+fhour+')',horizontalalignment='center',fontsize=6,transform=ax.transAxes,bbox=dict(facecolor='white',alpha=0.85,boxstyle='square,pad=0.2'))
        ax.imshow(im,aspect='equal',alpha=0.5,origin='upper',extent=(0,int(round(xmax*xscale)),0,int(round(ymax*yscale))),zorder=4)

      elif par == 3:
        cs = m.pcolormesh(x2,y2,snow_dif,cmap=cmdif,norm=normdif,ax=ax)
        ...
        ax.text(.5,1.03,para_str+' - '+prod_str+' Variable SLR Accumulated Snowfall ('+units+') \n initialized: '+itime+' valid: '+vtime + ' (f'+fhour+')',horizontalalignment='center',fontsize=6,transform=ax.transAxes,bbox=dict(facecolor='white',alpha=0.85,boxstyle='square,pad=0.2'))
        ax.imshow(im,aspect='equal',alpha=0.5,origin='upper',extent=(0,int(round(xmax*xscale)),0,int(round(ymax*yscale))),zorder=4)

      elif par == 4:
        snow_3[snow_3 > 1000.0] = 0    # avoid plotting missing values with pink
        cs_3 = m.pcolormesh(x4,y4,snow_3,cmap=cm,vmin=0.1,norm=norm,ax=ax)
...
#####################################
    ...
    snod_1 = snod_list_1[fhr] - snod_list_1[0]
    snod_2 = snod_list_2[fhr] - snod_list_2[0]
    snod_dif = snod_dif_list[fhr] - snod_dif_list[0]
    snow_3 = np.sum(np.array(snow_list_3[0:(fhr/6)+1]),axis=0)
    ...
    for ax in axes:
      ...
      if par == 1:
        cs_1 = m.pcolormesh(x,y,snod_1,cmap=cm,vmin=0.1,norm=norm,ax=ax)
        cs_1.cmap.set_under('white')
        cs_1.cmap.set_over('pink')
        cbar1 = m.colorbar(cs_1,ax=ax,location='bottom',pad=0.05,extend='max')
        cbar1.set_label(units,fontsize=6)
        ...
        cs_2 = m.pcolormesh(x2,y2,snod_2,cmap=cm,vmin=0.1,norm=norm,ax=ax)
        cs_2.cmap.set_under('white')
        cs_2.cmap.set_over('pink')
        cbar2 = m.colorbar(cs_2,ax=ax,location='bottom',pad=0.05,extend='max')
        cbar2.set_label(units,fontsize=6)
        ...
        cs = m.pcolormesh(x2,y2,snod_dif,cmap=cmdif,norm=normdif,ax=ax)
        cs.cmap.set_under('darkblue')
        cs.cmap.set_over('darkred')
        cbar3 = m.colorbar(cs,ax=ax,location='bottom',pad=0.05,extend='both')
        cbar3.set_label(units,fontsize=6)
    ...
    compress_and_save('comparesnod_'+dom+'_f'+fhour+'.png')
    t2 = time.clock()
    t3 = round(t2-t1, 3)
    print('%.3f seconds to plot accumulated snow depth for: '+dom) % t3
...
#################################
    ...
    if fhr == 0:
        ...
        snod6_1 = snod_list_1[fhr]*0.
        snod6_2 = snod_list_2[fhr]*0.
        snod6_dif = snod_dif_list[fhr]*0.
        snow6_3 = snow_list_3[fhr]*0.
    else:
        vtime_m6 = vtime_list[fhr-6]
        snod6_1 = snod_list_1[fhr] - snod_list_1[fhr-6]
        snod6_2 = snod_list_2[fhr] - snod_list_2[fhr-6]
        snod6_dif = snod_dif_list[fhr] - snod_dif_list[fhr-6]
    ...
    for ax in axes:
      ...
      if par == 1:
        cs_1 = m.pcolormesh(x,y,snod6_1,cmap=cm,norm=norm,ax=ax)
        ...
        cs_2 = m.pcolormesh(x2,y2,snod6_2,cmap=cm,norm=norm,ax=ax)
        ...
        cs = m.pcolormesh(x2,y2,snod6_dif,cmap=cmdif,norm=normdif,ax=ax)
        cs.cmap.set_under('darkblue')
        cs.cmap.set_over('darkred')
        cbar3 = m.colorbar(cs,ax=ax,location='bottom',pad=0.05,extend='both')
        cbar3.set_label(units,fontsize=6)
        ...
        cs_3 = m.pcolormesh(x4,y4,snow6_3,cmap=cm,norm=norm,ax=ax)
        cs_3.cmap.set_under('darkblue')
        cs_3.cmap.set_over('darkred')
        cbar4 = m.colorbar(cs_3,ax=ax,location='bottom',pad=0.05,extend='both')
        cbar4.set_label(units,fontsize=6)
        cbar4.ax.tick_params(labelsize=6)
    ...
    compress_and_save('comparesnod6_'+dom+'_f'+fhour+'.png')
    t2 = time.clock()
    t3 = round(t2-t1, 3)
